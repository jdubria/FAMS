...
function poppulate_list_faculty_schedule(){
    ...
    WHERE assign_schedule.sched_active = 1 AND semester.sem_id = {$sem_id} AND academic_year.ay_id = {$ay_id} GROUP BY fac_ID ";
...
function populate_schedule_day_time($ass_id){
    ...
    $output = ''; 
    $query = "
    SELECT * FROM assign_schedule WHERE ass_id = {$ass_id} AND sched_active = 1
    ";
    $result = mysqli_query($conn, $query);
    $data = mysqli_fetch_assoc($result);
    $day = $data['day'];
	...
	AND assign_schedule.start_time = '{$start_time}' AND assign_schedule.end_time = '{$end_time}' AND assign_schedule.sem_id = '{$sem_id}' AND academic_year.ay_id = '{$ay_id}' 
    ";
